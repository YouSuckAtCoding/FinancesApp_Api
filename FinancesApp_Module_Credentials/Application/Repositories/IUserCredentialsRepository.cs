...
public interface IUserCredentialsRepository
{
    ...
    Task UpdateUserSetCredentialsDate(Guid insertedId,
                                      SqlConnection? connection = null,
                                      CancellationToken token = default);
    Task<bool> SaveUserCredentialTotpAsync(UserCredentialsTotp totpCredentials,
                                           SqlConnection? connection = null,
                                           CancellationToken token = default);
    Task<bool> InvalidateUserCredentialTotpByIdAsync(Guid totpId,
                                                     SqlConnection? connection = null,
                                                     CancellationToken token = default);
    Task<bool> InvalidateUserCredentialTotpByUserIdAsync(Guid userId,
                                                         SqlConnection? connection = null,
                                                         CancellationToken token = default);
}

...
public class UserCredentialsTotpDomainTests
{
    ...
    public void RebuildFromEvents_Should_Restore_Created_State()
    {
        ...
        var createdEvent = new TotpCredentialCreatedEvent(Guid.NewGuid(), timestamp, userId, secret);
    ...
    {
        var userId = Guid.NewGuid();
        var createdEvent = new TotpCredentialCreatedEvent(Guid.NewGuid(), DateTimeOffset.UtcNow, userId, "JBSWY3DPEHPK3PXP");
        var invalidatedEvent = new TotpCredentialInvalidatedEvent(Guid.NewGuid(), DateTimeOffset.UtcNow, userId);
    ...
    public void ReadModel_Constructor_Should_Set_All_Properties()
    {
        ...
        var totp = new UserCredentialsTotp(userId, "SECRET", now, invalidAt, true);

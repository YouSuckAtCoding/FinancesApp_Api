...
namespace FinancesApp_Tests.Integration.UserTests.Repositories;
...
public class UserRepositoryTests : IClassFixture<SqlFixture>
{
    ...
    private const string TotpTableName = "[FinanceApp].[dbo].[UserCredentialsTotp]";
    ...
    public async Task CreateUserAsync_Should_Insert_User_To_Database()
    {
        ...
        {
            await ClearTotpTable(connection);
    ...
    private async Task ClearTotpTable(Microsoft.Data.SqlClient.SqlConnection connection)
    {
        await _commandFactory.ExecuteAsync(
            commandText: $"DELETE FROM {TotpTableName}",
            connection: connection,
            options: new CreateSqlCommandOptions(),
            operation: async command =>
            {
